...
FEED_TIMEOUT = 8   # seconds per feed before giving up

# ── Fetch RSS feeds ────────────────────────────────────────────────────────────
def fetch_single_feed(url: str) -> list:
    """Fetch one feed with a hard timeout."""
    def _timeout_handler(signum, frame):
        raise TimeoutError(f"Feed timed out: {url}")
...
def fetch_feeds(sources: dict) -> dict:
    ...
    for category, feeds in sources.items():
        ...
        for url in feeds:
            ...
            print(f"    {url} → {len(items)} articles")
...
# ── Build HTML page ────────────────────────────────────────────────────────────
def build_html(date: datetime.date, sections: dict) -> str:
    ...
    for key, data in sections.items():
        ...
        sections_html += f"""
        <section class="section depth-{meta['depth']}" id="{key}">
          <div class="section-header">
            <span class="section-icon">{meta["icon"]}</span>
            <h2>{meta["label"]}</h2>
            <span class="depth-badge">{meta["depth"]}</span>
          </div>
...
<head>
  ...
  <style>
    :root {{
      --bg: #0d0f0e; --surface: #131614; --border: #1e2320;
      --border-bright: #2a302c; --text: #c8d4c9; --text-dim: #5a6b5c;
      --text-bright: #e8f0e9; --accent: #4ade80; --accent-dim: #166534;
...
<body>
  ...
  <main>
    <div class="archive-bar">
      ARCHIVE <span>·</span> {{ARCHIVE_LINKS}}
    </div>
    {sections_html}
  </main>
  <footer>
...
def main():
    ...
    depth_map = {
        "bug_bounty": "deep", "hardware_hacking": "deep", "exploit_dev": "deep",
        "cve_threats": "moderate", "ai_cyber": "deep", "ai_tech": "moderate",
        "finance": "broad", "geopolitics": "broad", "science": "broad",
    }
    sections = {}
